...
namespace Cybersecurity_Awareness_Chatbot_Part2
{
    public class TaskManager
    {
        ...
        public List<CyberTask> GetAllTasks(string username = null)
        {
            ...
            try
            {
                string query = "SELECT * FROM tasks ORDER BY tasks_id DESC";
        ...
        private CyberTask MapReaderToTask(SqlDataReader reader)
        {
            try
            {
                if (reader["tasks_name"] == DBNull.Value || string.IsNullOrEmpty(reader["tasks_name"]?.ToString()))
                {
                    return null;
                }

                string status = reader["tasks_status"]?.ToString() ?? "pending";
                ...
                return new CyberTask
                {
                    ...
                    Title = reader["tasks_name"]?.ToString() ?? "Untitled Task",
            ...
            {
                logger?.LogError($"Error mapping reader to task: {ex.Message}", ex.StackTrace);
                return null;
            }
        }
    }
